...
/* -----------------------------------------------------------------------------
 ...
 -- AS isim vermemize yarar
 -- SUM toplamlarýný bulur.
 -- sirketler.sirket_adi, burda sirket_adinin srketlerden geldigi belirtir.
...
-----------------------------------------------------------------------------*/
    SELECT sirket_adi, personel_sayisi, (SELECT ROUND(AVG(maas),1) FROM personel
                                      WHERE sirket=sirketler.sirket_adi) AS ort_maas 
                                    
    FROM sirketler;
    
    -- AVG ortalamayi hesaplar
    -- (SELECT ROUND(AVG(maas),1) BURADA ROUND komutu yuvarlama yapar 
    -- , den sonra 1 rakam yazar
...
/* -----------------------------------------------------------------------------
 ...
 -------------------------------------------------------------------------*/
    ...
    -- bu soruda her dis dongu 1 kere calisir ic dongu ne kadar bilgi varsa o kadar calisir.
    -- COUNT kactane oldugunu sayar
