 ...
 /*==================== SET OPERATORLERI: UNION, UNION ALL======================
    ...
    ***  2 veya daha fazla tablodaki sutunleri birlestirmeye yarar  
==============================================================================*/ 
...
------------------------------------------------------------------------------*/
    ...
    FROM personel
    WHERE maas > 4000
    UNION                   -- UNION ALL kullanilirsa herseyiyle ayni olan sutunleridie alir
                                    -- tekrar edilenlerde gelir
    SELECT sehir, maas 
    FROM personel
    WHERE maas > 5000;
    
  * -----------------------------------------------------------------------------
  ORNEK1a: Maasi 4000’den cok olan isci isimlerini ve 5000 liradan fazla maas
...
------------------------------------------------------------------------------*/
    ...
    FROM personel
    WHERE maas > 4000
    UNION ALL                  -- UNION ALL kullanilirsa herseyiyle ayni olan sutunleridie alir
                                    -- tekrar edilenlerde gelir
    SELECT sehir, maas 
    FROM personel
    WHERE maas > 5000;   
...
* -----------------------------------------------------------------------------
  ...
  sorguyu yaziniz.   
------------------------------------------------------------------------------*/    
    SELECT maas,isim AS isim_veya_sehir  -- AS ile sutuna yeni isim vermis olduk
    FROM personel
    ...
    ORDER BY maas DESC;              -- yuksekten alcaga dogru siralar
...
?
/* -----------------------------------------------------------------------------
  ORNEK3: id’si 123456789 olan personelin Personel tablosundan sehir ve 
  maasini, personel_bilgi tablosundan da tel ve cocuk sayisini yazdirin  
------------------------------------------------------------------------------*/  
    SELECT sehir  AS tel_sehir,maas AS maas_cocuk_sayisi
    FROM personel   
    WHERE id = 123456789
    UNION    
...
------------------------------------------------------------------------------*/
-- 1. Yontem (maasi 5000 den az olanlar ile, sirketi Honda olmayanlarin (!=) sec) 
--     INTERSECT ile
...
------------------------------------------------------------------------------*/
    ...
    FROM personel
    WHERE isim = 'Mehmet Ozturk'
    MINUS                           -- Sehir istanbul olmayan
    SELECT isim,sehir
    FROM personel
    WHERE sehir = 'Istanbul';
    
    -- MINUS'da UNION gibi tekrarli satirlardan saedece bir tanesini listeler.
